...
void makeScene(int f) {
    ...
    spheres[0] = Sphere(0.3, vec3(.7, .0, -2.), vec3(1, 0, 0), 0);
    spheres[1] = Sphere(0.3, vec3(-.7, .0, -2.), vec3(0, 1, 0), 1);
    //spheres[3] = Sphere(0.8, vec3(1., 1, -5.5), vec3(0, 0, 1));
    //lights[0] = Light(vec3(0.0, 0.0, 0.0), vec3(1, 1, 1));
    spheres[2] = Sphere(0.2, vec3(.0, .0, -2.), vec3(1.0, 1.0, 1.0), 2);
    spheres[3] = Sphere(2., vec3(.0, .0, -6.), vec3(0., 0., 1.0), 1);
}
   ...
   for (int i=0; i<1*numSamples; i++) {
       ...
       for (int k=0; k<100; k++) {
            ...
            if (iSphere > -1) { 
                ...
                samp = samp + attenuation * vec3(.5, .5, .5) * bgColor(ray.direction);
                break;
            }
        }
   }
